...
exports.list= function(req, res) {
    ...
    //var page = (req.params['page'] > 0 ? req.params['page'] : 1) - 1;
    var perPage = (config.perPage)?config.perPage:20;
    var options = {
        perPage: perPage,
        page: page,
        criteria:null
    ...
    if (req.query && req.query.searchStr){
        ...
        var searchStr = req.query.searchStr.substring(0,50);
        var query = RegExp(searchStr, "i");
        //console.log();
        options.criteria={'name.ru': query}
        console.log(options);
        Stuff.list(options, function(err, stuffs) {
            if (err)  return res.json(err);
            if (page==0){
    ...
    else if (req.query && req.query.sale ){

        if (req.query.sale=='sale'){
            options.criteria={'tags': config.tags[0]}
        } else if (req.query.sale=='new'){
            options.criteria={'tags': config.tags[1]}
        } else if (req.query.sale=='superMaxi'){
            options.criteria={'tags': '53ac28a40aaec89b11e06d24'}
        }
    ...
    else if (req.query && req.query.main && req.query.main=='main'){
        options.criteria={'tags': '53adaeaeb2a3fba43a6ee805'};
...
exports.get= function(req, res) {
    //console.log(req.body);
    if (req.query && req.query.page && req.query.page=='page' ){
