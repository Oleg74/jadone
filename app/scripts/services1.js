    ...
    .provider('chats', function () {
       ...
       return {
            $get: function(socket,Chat,$rootScope,$timeout) {
                ...
                // init who are in chat now?
                function whoIsOnChat(){
                    //console.log('who is in chat');
                    socket.emit('who is in chat',$rootScope.user._id,function(res){
                       // console.log(res);
                        if (res){
                            for (var i=0;i<res.length;i++){
                                if ($rootScope.user._id!=res._id){
                                    usersOnLine.push(res[i]);
                                }
                            }
                        }
                    });
                ...
                $rootScope.$watch('user',function(newVal,oldVal){
                    ...
                    $timeout(function(){
                        if ($rootScope.user._id){
                            ...
                            socket.emit('new user in chat',$rootScope.user._id,function(data){ // remove cb for production

                                //console.log(data)
                            });
                            $timeout(function(){whoIsOnChat();},500);
                        } else {
                ...
                /*socket.on('notRead:msg',function(data){
                    for (var i=0;i<chatList.length;i++){
                        if(chatList[i]._id==data.from){
                            chatList[i].newMsg=true;
                        }
                    }
                ...
                socket.on('delete user from chat',function(data){
                    for (var i=0;i< usersOnLine.length;i++){
                        if(usersOnLine[i]._id==data){
                            usersOnLine.slice(i,1);
                            break;
                        }
                    }
                    // delete user drom list of chat on line
                })
               ...
               socket.on('new:msg',function(data,cb){
                    if ($rootScope.$state.includes('language.chat')){
                        ...
                        var from= data.from;
                        if (msgs[from]){
                            msgs[from].push({name:activeChat.name,msg:data.msg,date:data.date,class:false});
                            cb({cb:'cb from chat controller have read',status:true});
                        } else {
                          /* var tChat=findById(chatList,from);
                            tChat.newMsg++;*/

                            if (_.findWhere(chatList, {_id: from})){
                                _.findWhere(chatList, {_id: from}).newMsg++;
                            ...
                            //console.log(chatList);
                            cb({cb:"cb from chat controller don't read",status:false});
                        }
                    } else{
                        /*console.log(data);
                        console.log(chatList);
                        console.log(_.findWhere(chatList, {_id: from}));*/
                            // add chat
                ...
                function changeChat(chat,cb){
                    ...
                    /*for (var i=0;i<chatList.length;i++){
                        if(chatList[i]._id==chat._id){
                            chatList[i].newMsg=0;
                        }
                    }*/
                    Chat.list({from:$rootScope.user._id,to:activeChat._id},function(res){
                        //console.log(res);
                        var arr=[];
